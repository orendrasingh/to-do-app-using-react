        ...
        <div className="todo-section">
          ...
          {this.state.list.map(item =>{
            return(
              ...
              <input type="checkbox" name="isDone" className="task-checkbox" checked={item.done} onChange={e => this.updatedItem(e.target.value)} /><label>{item.task}</label><button className="delete-task" onClick={()=> this.deleteItem(item.id)}>Delete</button>
            </div>
            )
          })}
          </div>
