      ...
      <div>
        ...
        <div className="todo-section">
          ...
          {this.state.list.map(item =>{
            return(
              <div>
              <input type="checkbox" name="isDone" className="task-checkbox" checked={item.done} onChange={()=>{}} /><lable>{item.value}</lable><button className="delete-task" onClick={()=> this.deleteItem(item.id)}>Delete</button>
            </div>
            )
          })}
          </div>
