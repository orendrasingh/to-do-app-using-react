        ...
        <div className="todo-section">
          ...
          {this.state.list.map(item =>{
            return(
              ...
              <input type="checkbox" name="isDone" className="task-checkbox" checked={item.done} onChange={e => this.e.target.isDone} /><label>{item.task}</label><button className="delete-task" onClick={()=> this.deleteItem(item.id)}>Delete</button>
            </div>
            )
          })}
          </div>
