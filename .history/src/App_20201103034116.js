      ...
      <div className="Maindiv">
        ...
        <div className="todo-section">
          <div className="input-fields">
            <label >Enter the task</label><br></br>
          ...
          {this.state.list.map(item =>{
            return(
              <div key={item.id}>
              <input type="checkbox" name="isDone" className="task-checkbox" checked={item.done} onChange={()=>{}} /><label>{item.task}</label><button className="delete-task" onClick={()=> this.deleteItem(item.id)}>Delete</button>
            </div>
            )
          })}
          </div>
